...
\subsection{General-purpose MLIP benchmarking}
...
shown in Fig.~\ref{fig_comparison_dual_2x3}. Our NEP model, trained with an
energy-dependent scaling strategy, exhibits consistently superior performance
over tabGAP.
...
(2) relatively stable configurations (with average energies within
0.5~eV/atom of the $\beta$ phase).
...
THIS paragraph will discuss the comparison of computational speed between NEP and tabGAP. 
We will find out that the NEP dominates the tabGAP in computational speed, which also 
introudces a question that why NEP brokes the trade-off effect by being 
...
\textbf{(a)} Illustration of the energy-dependent rescaling strategy, where the black curve
...
energy-dependent rescaling, and NEP without energy-dependent rescaling over different
energy ranges.
}
\end{figure}

To clarify this ``pseudo-paradox'', we thoroughly compared the prediction errors of NEP and
tabGAP over all 22 configuration types in the GAP dataset
...
significantly lower occurrence probabilities, whereas the tabGAP training treats them equally.

In contrast, the NEP model exhibits weaker fluctuations in energy prediction accuracy
across different configuration types.
Our NEP is trained using a combined energy-dependent and configuration-type weighting strategy,
with parameters carefully tuned to balance the emphasis on physically important configuration
types and thermodynamics-informed rescaling.
This balance substantially reduces the MAE for many nonstoichiometric and amorphous
configurations, while introducing only a limited increase in the MAE for crystalline phases.
...
In contrast, the improvement in virial prediction is the most pronounced
...
forces, and virials is primarily attributed to the energy-dependent rescaling strategy,
as illustrated in Fig.~\ref{fig_energy_mae_comparison}(a).
The rescaling factors remain close to unity for the most stable configurations, decrease
...
The energy-prediction performances of tabGAP, NEP trained with energy-dependent rescaling,
and NEP trained without energy-dependent rescaling over different energy ranges are shown in
Fig.~\ref{fig_energy_mae_comparison}(b).
Without energy-dependent rescaling, NEP outperforms tabGAP only for configurations whose
average energies exceed those of the ambient $\beta$ phase by more than 0.5~eV/atom.
In contrast, with the energy-dependent rescaling scheme, NEP surpasses tabGAP across the
entire energy range, at the cost of only a decrease in accuracy for extremely
thermodynamically rare high-energy configurations.

\subsection{Physical process-oriented training set augmentation}
